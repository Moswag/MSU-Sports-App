...
//    checkUserExist(user.userId).then((value) {
//      if (!value) {
//        if(user.shopId.contains(SportsConstants.SHOP_WHOLESALE)){
//          user.role=SportsConstants.SALES_PERSON_WHOLESALE;
//          user.shopId=user.shopId.replaceAll(SportsConstants.SHOP_WHOLESALE,''.trim());
//          int x=user.shopId.length-1;
//          int y=user.shopId.length;
//          user.shopId=user.shopId.replaceRange(x,y,'');   //replace the space in between the end
//        }
//        else if(user.shopId.contains(SportsConstants.SHOP_FRENCHISE)){
//          user.role=SportsConstants.FRENCHISE;
//          user.shopId=user.shopId.replaceAll(SportsConstants.SHOP_FRENCHISE,''.trim());
//          int x=user.shopId.length-1;
//          int y=user.shopId.length;
//          user.shopId=user.shopId.replaceRange(x,y,'');   //replace the space in between the end
//        }
//        else if(user.shopId.contains(SportsConstants.SHOP_CAGE)){
//          user.role=SportsConstants.SALES_PERSON_AT_CAGE;
//          user.shopId=user.shopId.replaceAll(SportsConstants.SHOP_CAGE,''.trim());
//          int x=user.shopId.length-1;
//          int y=user.shopId.length;
//          user.shopId=user.shopId.replaceRange(x,y,'');   //replace the space in between the end
//        }
...
//  }
  ...
  static void addStudentSettingsDB(User user) async {
    checkUserExist(user.userId).then((value) {
      if (!value) {
        print("user ${user.name} ${user.email} added");
        Firestore.instance
